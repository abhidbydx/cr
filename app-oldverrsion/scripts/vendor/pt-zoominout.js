...
(function($) {
	...
	function animate_singular_text(elem, current_obj, options) {
		...
		var opacity_aux = 1;
		if (current_obj.isVideoPlaying == true) opacity_aux = 0;
		elem.animate({
			opacity: opacity_aux,
			left: leftPos + 'px',
			top: topPos + 'px'
	...
	function animate_texts(current_obj, options, bannerscollection_zoominout_the, bannerControls) {
		...
		thetexts.each(function() {
			...
			setTimeout(function() {
				animate_singular_text(currentText, current_obj, options);
			}, (currentText_arr[i].attr('data-delay') * 1000));
	...
	function initialPositioning(cur_i, options, bannerscollection_zoominout_container, origImgsDimensions, imgs) {
		...
		var w = parseInt(cur_initialZoom*origDim[0],10);
			var h = parseInt(cur_initialZoom*origDim[1],10);
		if(options.width100Proc && options.height100Proc){
			imgCurInside.css({
				"width":parseInt(cur_initialZoom*origDim[0],10)+"px",
				"height":parseInt(cur_initialZoom*origDim[1]-70,10)+"px"								
			});
		}
		else{
			if(!options.showBottomNav){
				...
				imgCurInside.css({
					"width":'auto',
					"height":parseInt(options.height - 62, 10)+"px"
				});
			}
		}	
	...
	function bannerscollection_zoominout_animate_image(current_obj, options, bannerscollection_zoominout_container, origImgsDimensions, imgs) {
		...
		if (current_initialZoom != current_finalZoom) {
			if (ver_ie != -1 && ver_ie < 10) {
				if (options.width100Proc) current_duration = current_duration + options.durationIEfix;
				...
				current_obj.msiInterval = setInterval(function() {
					...
					if ((nowx - current_obj.msiInitialTime) > (current_duration * 1000)) {
						...
						if (current_initialZoom <= current_finalZoom) current_obj.curZoom = 1 + zoomStep;
						else current_obj.curZoom = 1 - zoomStep;
						...
						current_obj.current_imgInside.css({
							filter: 'progid:DXImageTransform.Microsoft.Matrix(FilterType="bilinear",M11=' + current_obj.curZoom + ', M12=0, M21=0, M22=' + current_obj.curZoom + ', Dx=' + current_obj.cur_marginLeft + ',Dy=' + current_obj.cur_marginTop + ')'
						});
					}
				}, 25);
			} else {
	...
	function bannerscollection_zoominout_navigation(direction, current_obj, options, total_images, bottomNavButs, imgs, bannerscollection_zoominout_the, bannerControls, bannerscollection_zoominout_contentHolder, bannerscollection_zoominout_container, bannerscollection_zoominout_playOver, origImgsDimensions, thumbsHolder_Thumbs, bannerscollection_zoominout_thumbsHolder, bannerscollection_zoominout_carouselLeftNav, bannerscollection_zoominout_carouselRightNav, thumbsHolder_Thumb) {
		...
		if ((!options.loop && current_obj.current_img_no + direction >= total_images) || (!options.loop && current_obj.current_img_no + direction < 0)) navigateAllowed = false;
		...
		if (navigateAllowed && !current_obj.slideIsRunning) {
			...
			//destroy panzoom controllers on previous image
			if (current_obj.current_imgInside && current_obj.current_imgInside.panzoom) {
				current_obj.current_imgInside.panzoom("reset");
				current_obj.current_imgInside.panzoom("destroy");
			}
			...
			if (options.fadeSlides && total_images > 1) {
				...
				$('#contentHolderUnit_' + current_obj.previous_current_img_no, bannerscollection_zoominout_container).animate({
					...
					if (options.fadeSlides) {
						...
						var currentImg = $('#contentHolderUnit_' + current_obj.current_img_no, bannerscollection_zoominout_container).find('img:first');
						var ptZoomInOutControls = bannerscollection_zoominout_container.find('.ptZoomInOutControls');
						if (options.ptzoomControlVisible){
							if (currentImg[0].complete) {
								if (currentImg && currentImg.panzoom) {
									currentImg.panzoom({
										$zoomIn: ptZoomInOutControls.find(".zoom-in"),
										$zoomOut: ptZoomInOutControls.find(".zoom-out"),
										$zoomRange: ptZoomInOutControls.find(".zoom-range"),
										$reset: ptZoomInOutControls.find(".reset")
									});
								}
							} else {
					...
					if (current_obj.currentImg.attr('data-video') == 'true') bannerscollection_zoominout_playOver.css('display', 'block');

					//reinit content to stop videos
					if ($(imgs[current_obj.previous_current_img_no]).attr('data-video') == 'true') $('#contentHolderUnit_' + current_obj.previous_current_img_no, bannerscollection_zoominout_container).html($(imgs[current_obj.previous_current_img_no]).html());
	...
	function disableCarouselNav(current_obj, bannerscollection_zoominout_thumbsHolder, bannerscollection_zoominout_carouselLeftNav, bannerscollection_zoominout_carouselRightNav, options, total_images, thumbsHolder_Thumb) {
		...
		if (currentCarouselLeft < 0) {
			if (bannerscollection_zoominout_carouselLeftNav.hasClass('carouselLeftNavDisabled')) bannerscollection_zoominout_carouselLeftNav.removeClass('carouselLeftNavDisabled');
		} else {
		...
		if (Math.abs(currentCarouselLeft - current_obj.carouselStep) < (thumbsHolder_Thumb.width() + current_obj.thumbMarginLeft) * total_images) {
			if (bannerscollection_zoominout_carouselRightNav.hasClass('carouselRightNavDisabled')) bannerscollection_zoominout_carouselRightNav.removeClass('carouselRightNavDisabled');
		} else {
...
/*responsiveWidth=bannerscollection_zoominout_the.parent().parent().width();
		...
		if (options.origWidth != responsiveWidth || options.width100Proc) {
			...
			if (!options.height100Proc) options.height = options.width / current_obj.bannerRatio;

			if (options.enableTouchScreen && options.responsive && options.fadeSlides) options.width -= 1;
	...
	$.fn.bannerscollection_zoominout = function(options) {
		...
		return this.each(function() {
			...
			var bannerscollection_zoominout_wrap;
			var bannerControlsDef;
			var bannerscollection_zoominout_container;
			var bannerControls;
			var bannerscollection_zoominout_contentHolderVisibleWrapper;
			var bannerscollection_zoominout_contentHolder;
			var bottomNav_aux;
			var bannerscollection_zoominout_leftNav;
			var bannerscollection_zoominout_rightNav;
			var bannerscollection_zoominout_bottomNav;
			var bannerscollection_zoominout_bottomOverThumb;
			var bannerscollection_zoominout_thumbsHolderWrapper;
			var bannerscollection_zoominout_thumbsHolderVisibleWrapper;
			var bannerscollection_zoominout_thumbsHolder;
			var bannerscollection_zoominout_carouselLeftNav;
			var bannerscollection_zoominout_carouselRightNav;
			var ver_ie = getInternetExplorerVersion();
			var bannerscollection_zoominout_playOver;
			...
			function init() {
				origImgsDimensions = new Array();
				previousBottomHovered = 0;
				i = 0;
				total_images = 0;
				holderWidth = 0;
				bottomNavWidth = 0;
				bottomNavMarginTop = 0;
				thumbsHolder_MarginTop = 0;
				myopacity = 0;
				newTextLeft = 0;
				...
				if (options.responsiveRelativeToBrowser) {
						responsiveWidth=$(window).width();
						responsiveHeight=$(window).height();
					//responsiveWidth = bannerscollection_zoominout_the.parent().parent().parent().parent().parent().width();
					//responsiveHeight = bannerscollection_zoominout_the.parent().parent().parent().parent().parent().height();
				}
				options.origWidth = options.width;
				if (options.width100Proc) options.width = responsiveWidth;
				...
				if (options.responsive && (options.origWidth != responsiveWidth || options.width100Proc)) {
					...
					if (!options.height100Proc) options.height = options.width / (options.origWidth / options.origHeight);
				}
				...
				//the controllers
				if (!bannerscollection_zoominout_the.parent().hasClass('bannerscollection_zoominout')) {
					bannerscollection_zoominout_wrap = $('<div></div>').addClass('bannerscollection_zoominout').addClass(options.skin);
					bannerscollection_zoominout_the.wrap(bannerscollection_zoominout_wrap);
				}
				bannerControlsDef = $('<div class="bannerControls">   <div class="leftNav"></div>   <div class="rightNav"></div>      </div>  <div class="contentHolderVisibleWrapper"><div class="contentHolder"></div></div>   <div class="playOver"></div>  <div class="thumbsHolderWrapper"><div class="thumbsHolderVisibleWrapper"><div class="thumbsHolder"></div></div></div> <canvas class="mycanvas"></canvas>');
				bannerscollection_zoominout_the.after(bannerControlsDef);
				ptZoomInOutControls = $('<div class="ptZoomInOutControls buttons">' + '<button class="zoom-in fa fa-search-plus"></button>' + '<button style="display:none;" class="zoom-out fa fa-search-minus"></button>' + '<input type="range" class="zoom-range">' + '<button class="reset fa fa-undo"></button>' + '</div>');
				bannerscollection_zoominout_the.after(ptZoomInOutControls);
				if (!options.ptzoomControlVisible) {
					$('.ptZoomInOutControls', bannerscollection_zoominout_container).css("display", "none");
				}


				//the elements
				bannerscollection_zoominout_container = bannerscollection_zoominout_the.parent('.bannerscollection_zoominout');
				bannerControls = $('.bannerControls', bannerscollection_zoominout_container);


				bannerscollection_zoominout_contentHolderVisibleWrapper = $('.contentHolderVisibleWrapper', bannerscollection_zoominout_container);
				bannerscollection_zoominout_contentHolder = $('.contentHolder', bannerscollection_zoominout_container);


				bottomNav_aux = $('<div class="bottomNav"></div>');

				bannerscollection_zoominout_the.after(bottomNav_aux);

				if (!options.showAllControllers) bannerControls.css("display", "none");


				bannerscollection_zoominout_leftNav = $('.leftNav', bannerscollection_zoominout_container);
				bannerscollection_zoominout_rightNav = $('.rightNav', bannerscollection_zoominout_container);
				...
				$(document).on('keydown', function (e) {
					if (e.keyCode == 37) {
						debugger
						bannerscollection_zoominout_leftNav = $('.leftNav', $('.bannerscollection_zoominout')).slice(-1);
						bannerscollection_zoominout_leftNav.trigger('click');
					}

					if (e.keyCode == 39) {
						bannerscollection_zoominout_rightNav = $('.rightNav', $('.bannerscollection_zoominout')).slice(-1);
						bannerscollection_zoominout_rightNav.trigger('click');
					}
				});

				bannerscollection_zoominout_bottomNav = $('.bottomNav', bannerscollection_zoominout_container);
				...
				//thumbs
				bannerscollection_zoominout_thumbsHolderWrapper = $('.thumbsHolderWrapper', bannerscollection_zoominout_container);
				bannerscollection_zoominout_thumbsHolderVisibleWrapper = $('.thumbsHolderVisibleWrapper', bannerscollection_zoominout_container);
				bannerscollection_zoominout_thumbsHolder = $('.thumbsHolder', bannerscollection_zoominout_container);

				bannerscollection_zoominout_carouselLeftNav;
				bannerscollection_zoominout_carouselRightNav;
...
/*revert: true,
				...
				//the vars
				bannerscollection_zoominout_playOver = $('.playOver', bannerscollection_zoominout_container);
				bannerscollection_zoominout_playOver.css({
					'left': parseInt((options.width - bannerscollection_zoominout_playOver.width()) / 2, 10) + 'px',
					'top': parseInt((options.height - bannerscollection_zoominout_playOver.height()) / 2, 10) + 'px'
				});
			...
			function populateImages() {
				theul = bannerscollection_zoominout_the.find('ul:first');
				imgs = theul.children();
				...
				imgs.each(function() {
					...
					if (current_obj.currentImg.is('li')) {
						...
						//Nakul: width and height was not coming on scroll
						if (options.width100Proc || options.height100Proc) {
							//Do nothing
						} else {
							if (!imgInside.width()) {
								imgInside.css('width', options.width);
							}
							if (!imgInside.height()) {
								imgInside.css('height', options.height);
							}
						}
						//
			...
			function populateThumbs() {
				...
				if (options.skin != "opportune") {
					...
					img_inside = $('.thumbsHolder_ThumbOFF', bannerscollection_zoominout_container).find('img:first');
					img_inside.css("margin-top", thumbsHolder_MarginTop + "px");
					options.origthumbsHolder_MarginTop = thumbsHolder_MarginTop;

				}

				thumbsHolder_Thumbs = $('.thumbsHolder_ThumbOFF', bannerscollection_zoominout_container);
				rearangethumbs(current_obj, options, total_images, bannerscollection_zoominout_container, thumbsHolder_Thumbs, bannerscollection_zoominout_thumbsHolder, bannerscollection_zoominout_carouselLeftNav, bannerscollection_zoominout_carouselRightNav, thumbsHolder_Thumb, bannerscollection_zoominout_thumbsHolderVisibleWrapper, bannerscollection_zoominout_thumbsHolderWrapper);
			...
			$("iframe", bannerscollection_zoominout_container).each(function() {
				...
				if (ifr_source.indexOf('?') != -1) wmode = "&wmode=transparent";
...
/*if (options.fadeSlides) {
			...
			var ptZoomInOutControls = $('.ptZoomInOutControls', bannerscollection_zoominout_container);
			if (options.ptzoomControlVisible) {
				if (firstImg[0].complete) {
					if (current_obj.current_imgInside && current_obj.current_imgInside.panzoom) {
						current_obj.current_imgInside.panzoom({
							$zoomIn: ptZoomInOutControls.find(".zoom-in"),
							$zoomOut: ptZoomInOutControls.find(".zoom-out"),
							$zoomRange: ptZoomInOutControls.find(".zoom-range"),
							$reset: ptZoomInOutControls.find(".reset")
				...
				$('.bannerscollection_zoominout').find('.zoom-in').on('click', function (e) {
					e.preventDefault();
					if (current_obj.current_imgInside && current_obj.current_imgInside.panzoom) {
						current_obj.current_imgInside.panzoom('zoom', {
							animate: true,
							focal: {
								clientX: $(current_obj.current_imgInside).width() * 1.8,
								clientY: $(current_obj.current_imgInside).height()
							}
						});
					}
				});
			...
			//Nakul: Added custom controls
			var redrawControl = $('<div style="width:0px; height:0px" class="redraw"><button style="width:0px; height:0px"  id="btnRedraw"></button></div>');
			bannerscollection_zoominout_the.after(redrawControl);


			var redrawControls = $('.redraw', bannerscollection_zoominout_container);

			redrawControls.find('#btnRedraw').click(function() {
				//alert('redraw called');
				//$.removeData(bannerscollection_zoominout_the);
				$(document).off('keydown');
				if (bannerscollection_zoominout_wrap) {
					bannerscollection_zoominout_wrap.remove();
					//bannerscollection_zoominout_wrap.remove();
					$.removeData(bannerscollection_zoominout_wrap);
					//bannerscollection_zoominout_the
					bannerscollection_zoominout_wrap.off();
					bannerscollection_zoominout_wrap = null;

				}
				if (bannerControlsDef) {
					bannerControlsDef.remove();
					$.removeData(bannerControlsDef);
					bannerControlsDef.off();
					bannerControlsDef = null;
				}
				if (ptZoomInOutControls) {
					ptZoomInOutControls.remove();
					$.removeData(ptZoomInOutControls);
					ptZoomInOutControls.off();
					ptZoomInOutControls = null;
				}
				if (bannerscollection_zoominout_container) {
					bannerscollection_zoominout_container.off();
				}
				if (bannerControls) {
					bannerControls.remove();
					$.removeData(bannerControls);
					bannerControls.off();
					bannerControls = null;
				}
				if (bannerscollection_zoominout_contentHolderVisibleWrapper) {
					bannerscollection_zoominout_contentHolderVisibleWrapper.remove();
					$.removeData(bannerscollection_zoominout_contentHolderVisibleWrapper);
					bannerscollection_zoominout_contentHolderVisibleWrapper.off();
					bannerscollection_zoominout_contentHolderVisibleWrapper = null;
				}
				if (bannerscollection_zoominout_contentHolder) {
					bannerscollection_zoominout_contentHolder.remove();
					$.removeData(bannerscollection_zoominout_contentHolder);
					bannerscollection_zoominout_contentHolder.off();
					bannerscollection_zoominout_contentHolder = null;
				}
				if (bannerscollection_zoominout_leftNav) {
					bannerscollection_zoominout_leftNav.remove();
					$.removeData(bannerscollection_zoominout_leftNav);
					bannerscollection_zoominout_leftNav.off();
					bannerscollection_zoominout_leftNav = null;
				}
				if (bannerscollection_zoominout_rightNav) {
					bannerscollection_zoominout_rightNav.remove();
					$.removeData(bannerscollection_zoominout_rightNav);
					bannerscollection_zoominout_rightNav.off();
					bannerscollection_zoominout_rightNav = null;
				}
				if (bannerscollection_zoominout_bottomNav) {
					bannerscollection_zoominout_bottomNav.remove();
					$.removeData(bannerscollection_zoominout_bottomNav);
					bannerscollection_zoominout_bottomNav.off();
					bannerscollection_zoominout_bottomNav = null;
				}
				if (bannerscollection_zoominout_bottomOverThumb) {
					bannerscollection_zoominout_bottomOverThumb.remove();
					$.removeData(bannerscollection_zoominout_bottomOverThumb);
					bannerscollection_zoominout_bottomOverThumb.off();
					bannerscollection_zoominout_bottomOverThumb = null;
				}
				if (bannerscollection_zoominout_thumbsHolderWrapper) {
					bannerscollection_zoominout_thumbsHolderWrapper.remove();
					$.removeData(bannerscollection_zoominout_thumbsHolderWrapper);
					bannerscollection_zoominout_thumbsHolderWrapper.off();
					bannerscollection_zoominout_thumbsHolderWrapper = null;
				}
				if (bannerscollection_zoominout_thumbsHolderVisibleWrapper) {
					bannerscollection_zoominout_thumbsHolderVisibleWrapper.remove();
					$.removeData(bannerscollection_zoominout_thumbsHolderVisibleWrapper);
					bannerscollection_zoominout_thumbsHolderVisibleWrapper.off();
					bannerscollection_zoominout_thumbsHolderVisibleWrapper = null;
				}

				if (bannerscollection_zoominout_thumbsHolder) {
					bannerscollection_zoominout_thumbsHolder.remove();
					$.removeData(bannerscollection_zoominout_thumbsHolder);
					bannerscollection_zoominout_thumbsHolder.off();
					bannerscollection_zoominout_thumbsHolder = null;
				}

				if (bannerscollection_zoominout_carouselLeftNav) {
					bannerscollection_zoominout_carouselLeftNav.remove();
					$.removeData(bannerscollection_zoominout_carouselLeftNav);
					bannerscollection_zoominout_carouselLeftNav.off();
					bannerscollection_zoominout_carouselLeftNav = null;
				}

				if (bannerscollection_zoominout_carouselRightNav) {
					bannerscollection_zoominout_carouselRightNav.remove();
					$.removeData(bannerscollection_zoominout_carouselRightNav);
					bannerscollection_zoominout_carouselRightNav.off();
					bannerscollection_zoominout_carouselRightNav = null;
				}

				if (bannerscollection_zoominout_playOver) {
					bannerscollection_zoominout_playOver.remove();
					$.removeData(bannerscollection_zoominout_playOver);
					bannerscollection_zoominout_playOver.off();
					bannerscollection_zoominout_playOver = null;
				}

				if (current_obj) {
					$.removeData(current_obj);
					current_obj = null;
				}

				if (origImgsDimensions) {
					$.removeData(origImgsDimensions);
					origImgsDimensions = null;
				}
				...
				if (redrawControls) {
					redrawControls.remove();
					$.removeData(redrawControls);
					redrawControls.off();
					redrawControls = null;
				}

				if (bannerscollection_zoominout_the) {
					$.removeData(bannerscollection_zoominout_the);
					bannerscollection_zoominout_the.off();
					bannerscollection_zoominout_the = null;
				}
		...
		if (options.skin == "opportune") {
			...
			if (options.thumbsWrapperMarginTop < 0 && current_obj.isVideoPlaying) {
				...
				if (options.showBottomNav) {
					...
					bannerscollection_zoominout_thumbsHolderWrapper.stop().animate({
						opacity: 1
					}, 500, 'swing', function() {
						//complete
					});
...
contentHolderUnit.click(function() {
	...
	if (mycurImg.attr('data-link') != undefined && i == current_obj.current_img_no && mycurImg.attr('data-link') != '') {
		...
		if (cur_target == "_blank") window.open(mycurImg.attr('data-link'));
		else window.location = mycurImg.attr('data-link');
	}
});
...
$(window).resize(function() {
	...
	if (navigator.userAgent.indexOf('Android') != -1) {
		if (options.windowOrientationScreenSize0 == 0 && window.orientation == 0) options.windowOrientationScreenSize0 = $(window).width();

		if (options.windowOrientationScreenSize90 == 0 && window.orientation == 90) options.windowOrientationScreenSize90 = $(window).height();

		if (options.windowOrientationScreenSize_90 == 0 && window.orientation == -90) options.windowOrientationScreenSize_90 = $(window).height();

		if (options.windowOrientationScreenSize0 && window.orientation == 0 && $(window).width() > options.windowOrientationScreenSize0) doResizeNow = false;

		if (options.windowOrientationScreenSize90 && window.orientation == 90 && $(window).height() > options.windowOrientationScreenSize90) doResizeNow = false;

		if (options.windowOrientationScreenSize_90 && window.orientation == -90 && $(window).height() > options.windowOrientationScreenSize_90) doResizeNow = false;
	...
	if (ver_ie != -1 && ver_ie == 9 && current_obj.windowWidth == 0) doResizeNow = false;
	...
	} else current_obj.windowWidth = $(window).width();
	...
	if (options.responsive && doResizeNow) {
		...
		TO = setTimeout(function() {
			doResize(current_obj, options, total_images, imgs, bannerscollection_zoominout_the, bannerControls, bannerscollection_zoominout_contentHolderVisibleWrapper, bannerscollection_zoominout_contentHolder, bannerscollection_zoominout_container, bannerscollection_zoominout_playOver, origImgsDimensions, thumbsHolder_Thumbs, bottomNavButs, bannerscollection_zoominout_thumbsHolder, bannerscollection_zoominout_carouselLeftNav, bannerscollection_zoominout_carouselRightNav, thumbsHolder_Thumb, bannerscollection_zoominout_leftNav, bottomNavBut, bannerscollection_zoominout_bottomNav, bannerscollection_zoominout_thumbsHolderVisibleWrapper, bannerscollection_zoominout_thumbsHolderWrapper)
...
if (options.skin == "opportune") {
	...
	bottomNavButs.mouseleave(function() {
		...
		if (current_obj.current_img_no != i) currentBut.removeClass('bottomNavButtonON');
	});
...
thumbsHolder_Thumbs.mouseleave(function() {
	...
	if (current_obj.current_img_no != i) currentBut.removeClass('thumbsHolder_ThumbON');
});
...
bannerscollection_zoominout_carouselLeftNav.click(function() {
	if (!current_obj.isCarouselScrolling) {
		...
		if (currentCarouselLeft < 0) carouselScroll(1, bannerscollection_zoominout_thumbsHolder, bannerscollection_zoominout_carouselLeftNav, bannerscollection_zoominout_carouselRightNav, options, total_images, thumbsHolder_Thumb, current_obj);
	}
});
...
bannerscollection_zoominout_carouselRightNav.click(function() {
	if (!current_obj.isCarouselScrolling) {
		currentCarouselLeft = bannerscollection_zoominout_thumbsHolder.css('left').substr(0, bannerscollection_zoominout_thumbsHolder.css('left').lastIndexOf('px'));
		if (Math.abs(currentCarouselLeft - current_obj.carouselStep) < (thumbsHolder_Thumb.width() + current_obj.thumbMarginLeft) * total_images) carouselScroll(-1, bannerscollection_zoominout_thumbsHolder, bannerscollection_zoominout_carouselLeftNav, bannerscollection_zoominout_carouselRightNav, options, total_images, thumbsHolder_Thumb, current_obj);
	}
});
...
if ($(imgs[current_obj.current_img_no]).attr('data-video') == 'true') bannerscollection_zoominout_playOver.css('display', 'block');
