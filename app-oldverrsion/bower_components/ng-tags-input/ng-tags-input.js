 ...
 * @param {string=} [tempPlaceholder=Add a tag] Custom Placeholder text to replace original
...
tagsInput.directive('tagsInput', ["$timeout","$document","tagsInputConfig", function($timeout, $document, tagsInputConfig) {
    function TagList(options, events) {
        ...
        self.add = function(tag) {
            ...
            //Extra check for maxtag added
            if (tagIsValid(tag) && self.items.length < options.maxTags) {
                self.items.push(tag);
    ...
    return {
        ...
        controller: ["$scope","$attrs","$element",function($scope, $attrs, $element) {
            tagsInputConfig.load('tagsInput', $scope, $attrs, {
                ...
                tempPlaceholder: [String, 'Add a tag'],
                tabindex: [Number],
        ...
        link: function(scope, element, attrs, ngModelCtrl) {
			...
			var ulElement = element.find('ul.tag-list');
			
            scope.$watch('tags', function(value) {
                scope.tags = makeObjectArray(value, options.displayProperty);
                tagList.items = scope.tags; 
                //Extra check if atleast one tag selected hide placeholder
                if(tagList.items.length >=1){
                    options.placeholder = '';
					ulElement.addClass('extra');
                }else{
                    options.placeholder = options.tempPlaceholder;
					ulElement.removeClass('extra');
                } 
 ...
 */
tagsInput.directive('autoComplete', ["$filter", "$document","$timeout","$sce","tagsInputConfig", function($filter, $document, $timeout, $sce, tagsInputConfig) {
    function SuggestionList(loadFn, options) {
        ...
        self.load = function(query, tags) {
            ...
            //$timeout.cancel(debouncedLoadId);
                ...
                var promise = loadFn({ $query: query }), items;
                lastPromise = promise;	
				//Apply filter for autocomple	 
				items = $filter('filter')(lastPromise, {'label': query}, false) 
    ...
    return {
        ...
        link: function(scope, element, attrs, tagsInputCtrl) {
            var hotkeys = [KEYS.enter, KEYS.tab, KEYS.escape, KEYS.up, KEYS.down],
                suggestionList, tagsInput, options, getItemText, documentClick, ucword;
            ...
            getItemText = function(item) {               
                return ucword(item[options.tagsInput.displayProperty]);
            };
